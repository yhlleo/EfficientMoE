...
from utils import update_ema, requires_grad, cleanup, setup_ddp, setup_exp_dir, setup_data, instantiate_from_config, get_lr_scheduler_config, wandb_initialize, wandb_log
from omegaconf import OmegaConf
from utils import fsdp_wrapper, fsdp_ema_setup, fsdp_ema_update, FSDPConfig
...
def main(config, use_wandb=True, resume_path=None, dtype="bf16"):
    ...
    ema = deepcopy(model).to("cpu")  # Create an EMA of the model for use after training
    ...
    ema = fsdp_ema_setup(ema, fsdp_config)
    ...
    for epoch in range(cur_epoch, config.basic.epochs):
        ...
        for x, y in loader:
            ...
            with torch.autocast("cuda", dtype=float_type):
                if config.basic.rf:
            ...
            if (global_step + 1) % accum_iter == 0:
                ...
                opt.step()
                opt.zero_grad()
                fsdp_ema_update(ema, model, decay=0.9999)
                train_steps += 1
            ...
            if global_step % (ckpt_every*accum_iter) == 0:
                ...
                with FSDP.state_dict_type(ema, StateDictType.FULL_STATE_DICT, full_cfg):
...
if __name__=="__main__":
    ...
    if args.wandb:
        ...
        experiment_name = args.exp_name #"train_b_e16_dsmoe-naive-v3"
        wandb_initialize(entity, experiment_name, project)

    main(config_yml_path, resume_path=args.resume_path, dtype=args.float_type)
